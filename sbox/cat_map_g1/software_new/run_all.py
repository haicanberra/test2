...
for k in range(dicom_global_params.K):
    for height in range(list_im_size[k][0]):
        for width in range(list_im_size[k][1]):
            temp = kC_cipher[k][height][width]
            temp_str = bf.sbox_subbyte(Sbox = dicom_global_params.Sbox_inv, bit_sequence = np.binary_repr(temp, width = dicom_global_params.NB_max))
            kC_cipher[k][height][width] = np.uint32(int(temp_str, 2))
